...
locals {
  processed_vpc_peering = var.vpc_peering_enabled ? [for peering in var.vpc_peering : {
    name         = peering.name
    network      = peering.network != null ? peering.network : google_compute_network.vpc.self_link
...
resource "google_compute_network" "vpc" {
  ...
  auto_create_subnetworks = var.vpc["default-subnet-creation"]
}
...
resource "google_compute_subnetwork" "subnet" {
  ...
  ip_cidr_range            = var.vpc["main-cidr"]
...
resource "google_compute_subnetwork" "proxy_subnet" {
  project                  = var.project_id
  name                     = var.vpc["proxy-subnet-name"]
  region                   = var.region
  network                  = google_compute_network.vpc.name
  ip_cidr_range            = var.vpc["proxy-subnet-cidr"]
...
resource "google_compute_firewall" "firewall_vpc" {
  count = length(var.vpc_firewall)

  project                 = var.project_name
  network                 = google_compute_network.vpc.id
  name                    = var.vpc_firewall[count.index].name
  priority                = var.vpc_firewall[count.index].priority
  source_service_accounts = var.vpc_firewall[count.index].source_service_accounts != null ? var.vpc_firewall[count.index].source_service_accounts : null
  source_ranges           = var.vpc_firewall[count.index].source_ranges != null ? var.vpc_firewall[count.index].source_ranges : null
  direction               = var.vpc_firewall[count.index].direction != null ? var.vpc_firewall[count.index].direction : 0
